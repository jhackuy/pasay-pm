...
```
   APPROVED_NOT_DISPATCHED (gate passed, awaiting OpenDesign)
       ...
       DISPATCHED <----+    (OpenDesign accepted)
            |          |
            v          |
       DESIGN_ACCEPTED |   (Owner accepts the design)
            |          |
            v          |
   (issue label may be  |
    flipped to          |
    ready-for-dev)      |
                       |
   DISPATCH_FAILED <----+    (OpenDesign unreachable / rejected)
...
2. **Public HTTP endpoint** for the OpenDesign daemon. Owner sets the
   repository secrets `OD_DISPATCH_URL` (base URL, e.g.
   `https://design.pasay.example.com`) and `OD_TOOL_TOKEN` (bearer).
   The dispatcher POSTs the dispatch input as JSON to
   `{OD_DISPATCH_URL}/api/opendesign/dispatch`.

Until either of these is set, the dispatcher will report
`BLOCKED_FOR_PRODUCT_DECISION` for every approval event. That is the
correct safety behaviour: the mechanism is wired and auditable; the
runtime transport is the only missing piece.
...
asserts the verdict + state. This is how CI proves the contract holds
before merge, since the real `issue_comment` event cannot be received
until the workflow is on the default branch.
...
`DISPATCH_FAILED` (endpoint unreachable) is exercised by re-running
the same fixture runner with `--mode reject`.

## Security notes

* Issue body / comment body are never interpolated into shell. They are
  stored as JSON strings inside `dispatch_input.issue.*` and parsed by
  the OpenDesign side.
* The approval marker is matched case-sensitively and as an exact
...
* The workflow declares the minimum permissions (`contents: read`,
  `issues: write`).

## Files

* `.github/workflows/opendesign-dispatch.yml` — the workflow.
