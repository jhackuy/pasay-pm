"""Entry B rent-payment matching service (V1.3 Slice 2, exact payments).
...
Scope of this slice: exact payments + duplicate recognition only. Partial /
overpayment / ambiguous / correction / query flows belong to later cards and
...
class LeaseCtx:
    ...
    def open_periods(self) -> list[tuple[str, date]]:
        return [(m, d) for m, d in self.periods if m not in self.covered and m not in self.pending_periods]
...
def _build_open_candidate(row, rows: list, hints: ParsedHints, resolved_amount: Optional[Decimal]) -> MatchCandidate:
    ...
    if resolved_amount is not None and resolved_amount != ctx.lease.monthly_rent.quantize(TWO_PLACES):
        confidence = MatchConfidence.LOW
    remaining = max(open_count - 1, 0) * ctx.lease.monthly_rent
    return MatchCandidate(
        ...
        period=row.period,
        due_date=row.due,
        amount=ctx.lease.monthly_rent.quantize(TWO_PLACES),
        open_count=open_count,
        remaining_balance=remaining.quantize(TWO_PLACES),
    )
...
def _match_existing(ctxs: list[LeaseCtx], hints: ParsedHints, resolved_amount: Optional[Decimal]) -> list[MatchCandidate]:
    ...
    the payment was already booked."""
    ...
    for ctx in ctxs:
        ...
        if resolved_amount is not None and resolved_amount != rent:
            continue
        for income in ctx.incomes:
            if income.status not in (IncomeStatus.pending, IncomeStatus.confirmed):
                continue
            if income.amount.quantize(TWO_PLACES) != rent:
                continue
...
def match_from_leases(
    ...
    if resolved_amount is not None:
        rows = [r for r in rows if r[0].lease.monthly_rent.quantize(TWO_PLACES) == resolved_amount]
