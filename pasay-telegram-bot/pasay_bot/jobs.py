...
async def _send_digest(app: Application, api, store: StateStore) -> None:
    try:
...
async def _send_next_check_reminders(app: Application, api, store: StateStore) -> None:
...
def register_jobs(
    ...
    async def _digest_job(context):
        await _send_digest(app, api, store)

    async def _next_check_job(context):
        await _send_next_check_reminders(app, api, store)
