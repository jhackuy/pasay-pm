...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # SLICE3-UX-PERSISTENT-MENU-002: menu initialization must not depend on
    # dashboard/backend success. Private chats get the persistent keyboard
    # even when the dashboard fetch failed (show_dashboard only sends inline
    # error keys on failure).
    chat = update.effective_chat
    if chat is not None and chat.type == Chat.PRIVATE:
        await _send_persistent_menu(context, chat.id, role, locale_for(role))
