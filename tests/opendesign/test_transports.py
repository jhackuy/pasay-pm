...
from scripts.opendesign.dispatch_http import HttpTransport  # noqa: E402
...
def test_stub_accept_records_attempt():
    ...
    assert ack["target"] == "D:\\AI-DESIGN\\projects\\pasay-stub"
...
def test_stub_appends_log_file(tmpdir=None):
...
# ---------------------------------------------------------------------------
# HTTP transport (purely structural: refused without base_url, etc.)
# ---------------------------------------------------------------------------
...
def test_http_transport_refuses_to_unreachable_host():
    # Use an unroutable host so urlopen fails fast.
    t = HttpTransport(
