...
def upgrade():
    op.create_table("principals",
        ...
        sa.CheckConstraint("principal_type IN ('HUMAN','SERVICE','AI_AGENT','SYSTEM')", name="principal_type"))
    ...
    op.create_table("api_credentials",
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False), sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("created_by", sa.BigInteger()), sa.Column("updated_by", sa.BigInteger()), sa.CheckConstraint("state IN ('ACTIVE','REVOKED')", name="credential_state"))
    ...
    op.create_table("credential_lifecycle_history",
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False), sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("created_by", sa.BigInteger()), sa.Column("updated_by", sa.BigInteger()), sa.CheckConstraint("state IN ('ACTIVE','REVOKED')", name="credential_history_state"))
