...
The runner is intentionally transport-agnostic. The transport layer is
provided by dispatch_http (HTTP / daemon URL) or dispatch_stub (PR-stage
fixture). Either way, the runner sees a single Transport object exposing
one method:

    transport.submit(dispatch_input) -> DispatchAck

DispatchAck is a simple dict:
...
The runner never builds a shell command from user-controlled text. The
only shell command it ever runs is `gh api` for writeback, and even that
is mediated by writeback helpers which encode the JSON safely.
"""
