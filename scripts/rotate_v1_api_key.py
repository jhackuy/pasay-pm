...
def _resolve_identity(
...
) -> tuple[User, list[ApiCredential], str, str | None]:
    """Resolve (user, actives, principal_type, purpose) for the rotation.
    ...
    if username:
        ...
        return user, actives, V1PrincipalType.HUMAN.value, None
    ...
    return user, actives, V1PrincipalType.SYSTEM.value, purpose
...
def main(argv: list[str] | None = None) -> int:
    ...
    db = factory()
    try:
        user, actives, pt, purpose = _resolve_identity(
        ...
        if args.username:
            ...
            scope = f"principal_type=SYSTEM purpose={args.purpose!r}"
